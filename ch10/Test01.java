...
public class Test01 {
	public static void main(String[] args) {
		...
		arr[0] = s;
		arr[1] = i1;
		// int len = arr[0].length(); //Object에 length()가 없다, arr[0]은 object
		int len = ((String) arr[0]).length(); // 타입변경

		ArrayList lst = new ArrayList();// 모든 타입의 객체들을 저장할 수 있다 그러나 비추다! - 이유는?
		String s2 = "java";
		Integer i2 = 123; //Wrapper클래스에 의해서 박싱
		lst.add(s2); //String객체
		lst.add(i2); //int객체
		...
		list.addAll(list2); //c의 타입은 E를 상속한 class의 객체들은 모두 가능
		print(13, list);

		// boolean containsAll(Collection<?> c) c의 타입은 모두 가능
